...
let validateName = (name) => {
    ...
    try {
        if (name != "" && name.length > 2) {
            //check if name is empty and has valid length
...
let validatePhone = (phone, blank = false) => {
    ...
    try {
        if (blank == true && phone == "") {
            //check if phone is empty and blank is true (for optional field)
            return true;
        }
...
try {
    ...
    form.addEventListener("submit", function (e) {
        ...
        formSubmitBtn.innerHTML = `<span class="py-3"><span
                                class="mr-1.5 inline-block h-3.5 w-3.5 animate-spin rounded-full border-2 border-solid border-current border-e-transparent align-[-0.125em]"
                                role="status"></span>Submitting...</span>`;
        ...
        if (
            validateName(name) &&
            validateEmail(email) &&
            validatePhone(phone, (blank = true))
        ) {
            //validate message
            if (message.length > 10) {
                if (message.length < 450) {
                    ...
                    // // ! Debug log
                    // console.log("Sending data:", contactFormData);

                    // !! PageClip is confined to specific domains only.
                    // Submit form using pageclip
                    Pageclip.send(
                        "xOvO19qYNv6JtHECcq060hSxyq3JDD7T",
                        "contact",
                        contactFormData,
                        function (error, response) {
                            // // ! Debug log
                            // console.log("Pageclip response:", response);

                            if (error) {
                                console.error("Pageclip Error:", error);
                                window.alert(
                                    `Failed to submit your record. \nTry again later.`
                                );
